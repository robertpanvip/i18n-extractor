 ...
 * 若直接把 [I18nFoldingBuilder] 注册到 Vue，会遮蔽链上原生的 XmlFoldingBuilder，
 * 导致 template 内 div / 标签等结构折叠全部失效（JS 语言不受影响，因为它原生已有
 * JSFoldingBuilder，多个 builder 会被平台合成 CompositeFoldingBuilder）。
 *
 * ## 合并策略
 *  - 结构折叠（div 等）委托原生 builder：从 root 语言的**基语言链**（跳过 Vue 自身）
 *    查找首个注册的折叠 builder，等价于未注册本扩展时 `forLanguage(Vue)` 沿链命中的结果；
 *  - `$t()/t()` 翻译折叠复用 [I18nFoldingBuilder]，其 descriptor 在构造时硬编码
 *    `collapsedByDefault=true`，不依赖本类的 [isCollapsedByDefault] 委托；
...
class VueHostFoldingBuilder : FoldingBuilderEx() {
    ...
    /**
     * 原生结构折叠 builder：沿 [language] 的基语言链（跳过自身，如 Vue → HTML → XML）
     * 取首个注册的折叠 builder。等价于未注册本扩展时平台 forLanguage(Vue) 的分发结果。
     * [LanguageFolding.forLanguage] 自带按语言实例的缓存，重复查询开销可忽略。
     */
    private fun nativeStructureBuilder(language: Language?): FoldingBuilder? {
        var lang = language?.baseLanguage ?: return null
        while (true) {
            LanguageFolding.INSTANCE.forLanguage(lang)?.let { return it }
            lang = lang.baseLanguage ?: return null
        }
    }

    override fun buildFoldRegions(root: PsiElement, document: Document, quick: Boolean): Array<FoldingDescriptor> {
        val byRange = LinkedHashMap<TextRange, FoldingDescriptor>()
        val native = nativeStructureBuilder(root.language)
        if (native != null) {
    ...
    /** 原生结构折叠的默认状态（div 等默认展开），按节点语言链委托原生 builder 判断。 */
    override fun isCollapsedByDefault(node: ASTNode): Boolean =
        nativeStructureBuilder(node.psi?.language)?.isCollapsedByDefault(node) ?: false

    override fun getPlaceholderText(node: ASTNode): String? =
        nativeStructureBuilder(node.psi?.language)?.getPlaceholderText(node)

    override fun getPlaceholderText(node: ASTNode, range: TextRange): String? {
        val native = nativeStructureBuilder(node.psi?.language) ?: return null
